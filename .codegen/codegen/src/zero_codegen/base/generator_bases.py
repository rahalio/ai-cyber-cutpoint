...
class FileGenerator(BaseGenerator):
    ...
    def generate(self, context: GenerationContext) -> GenerateResult:
        ...
        # Webapp generators share one domain folder across multiple steps — never
        # honor pipeline.clean here (layer_cleaner already wiped once per run).
        if str(getattr(self, "output_layer", "")).startswith("webapp"):
            should_clean = self.should_clean()
        else:
