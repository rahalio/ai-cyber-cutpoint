...
class HandlerGenerator(BaseGenerator):
    ...
    def _generate_handler_content(
        ...
        type_import_stmt = ""
        if type_imports:
            type_import_stmt = f'import type {{ {type_imports} }} from "@cutpoint/core/{context.domain_name}/types";\n'

        input_type_imports = ", ".join(sorted(input_type_names))
        input_import_stmt = f'import type {{ {input_type_imports} }} from "@cutpoint/services/{context.domain_name}";\n' if input_type_names else ""
    ...
    meta: {
      ...
      timestamp,
        ...
        if operation_id == "getVersion" or (path or "") == "/api/version":
            return """  let name = "@cutpoint/api-server";
  let version = "0.1.0";
  try {
