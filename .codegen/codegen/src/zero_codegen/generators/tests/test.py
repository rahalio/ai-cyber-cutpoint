...
"""
    ...
    def _generate_entity_factory(
        ...
        imports.append(f'import type {{ {resource_pascal}Entity }} from "@cutpoint/core/{context.domain_name}/models/index.js";')
        domain_schemas_name = f"{camel_case(context.domain_name)}Schemas"  # e.g., activitySchemas
        imports.append(f'import {{ {domain_schemas_name} }} from "@cutpoint/core/{context.domain_name}";')
...
"""
    ...
    def _generate_dto_factory(
        ...
        if is_params_type:
            imports.append(f'import type {{ {dto_schema_name} }} from "@cutpoint/core/{context.domain_name}";')
...
import type {{
  {repo_imports}
}} from "@cutpoint/core/{context.domain_name}/repositories/index.js";
import type {{
  PaginationParams,
  PaginatedResult,
}} from "@cutpoint/core/_shared/index.js";
...
// Mock repositories are not needed - use createMockDependencies from @cutpoint/tests/shared
...
"""
    ...
    def _generate_handler_test_content(
        ...
        handler_import_path = f"@cutpoint/api-server/domains/{context.domain_name}/handlers"
        ...
        if has_dto_factory and verb in ["create", "update", "patch"]:
            verb_pascal = pascal_case(verb)
            dto_factory_name = f"create{verb_pascal}{resource_pascal}Request"
            dto_factory_import = f'import {{ {dto_factory_name} }} from "@cutpoint/factories/{context.domain_name}";\n'

        # Import test factories from shared location (using barrel exports)
        test_factory_import = f'import {{ createMockRequest, createMockReply, createMockDependencies, TEST_ORG_ID }} from "@cutpoint/tests/shared";\n'
        ...
        entity_factory_import = f'import {{ create{resource_pascal} as create{resource_pascal}Entity }} from "@cutpoint/factories/{context.domain_name}";\n' if has_entity_factory else ""

        return f"""{header}
import {{ describe, it, expect, beforeEach, vi }} from "vitest";
import {{ {handler_name} }} from "{handler_import_path}";
import type {{ {domain_pascal}DDDDependencies }} from "@cutpoint/api-server/domains/{context.domain_name}";
...
"""
    ...
    def _generate_usecase_test_content(
        ...
        if needs_dto_factory:
            dto_factory_import = f'import {{ {dto_factory_name} }} from "@cutpoint/factories/{context.domain_name}/factories/{resource_kebab}/dto/{verb}-{resource_kebab}.dto.factory.js";\n'
        ...
        # Use kebab-case filename with -usecase suffix
        usecase_file_name = kebab_case(usecase_name)
        relative_path = f"@cutpoint/services/{context.domain_name}/usecases/{usecase_file_name}-usecase.js"

        return f"""{header}
import {{ describe, it, expect, beforeEach, vi }} from "vitest";
import {{ {usecase_name} }} from "{relative_path}";
import {{ {mock_repo_name} }} from "@cutpoint/factories/{context.domain_name}/mocks/index.js";
import {{ create{resource_pascal} as create{resource_pascal}Entity }} from "@cutpoint/factories/{context.domain_name}/factories/{resource_kebab}/entity/{resource_kebab}.entity.factory.js";
{dto_factory_import}import {{ TEST_ORG_ID }} from "@cutpoint/factories/{context.domain_name}/e2e/setup.js";
