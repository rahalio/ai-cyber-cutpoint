...
export function getCoreTableName(): string {
  const tableName =
    ...
    process.env.DYNAMODB_CORE_TABLE_NAME ||
    'cutpoint-sandbox-local';
  return tableName;
}
...
export function getBaseTableName(): string {
  const tableName =
    process.env.BASE_TABLE_NAME ||
    process.env.DYNAMODB_BASE_TABLE_NAME ||
    getCoreTableName();
  return tableName;
}
...
export function getAnalyticsTableName(): string {
  const tableName =
    ...
    process.env.DYNAMODB_ANALYTICS_TABLE_NAME ||
    getCoreTableName();
  return tableName;
}
