...
import type { CandidateRepository } from "@cutpoint/services/discovery";
 ...
 *
 * Entity Type: CHAIN_CANDIDATE
 * Entity Token: CHAIN_CANDIDATE
 * SK Prefix: CHAIN_CANDIDATE
 */
export class CandidateRepositoryDdb implements CandidateRepository {
  ...
  private readonly ENTITY_TYPE = "CHAIN_CANDIDATE";
  private readonly ENTITY_TOKEN = "CHAIN_CANDIDATE";
  private readonly SK_PREFIX = "CHAIN_CANDIDATE";
  ...
  private readonly ENTITY_TYPE_PK = "CHAIN_CANDIDATES";
  ...
  async getChainCandidate(input: Parameters<CandidateRepository['getChainCandidate']>[0]): Promise<Awaited<ReturnType<CandidateRepository['getChainCandidate']>>> {
    ...
    if (!(inputExt.candidateId ?? inputExt.id ?? inputExt.candidateId)) {
      throw new Error("Missing required parameter: candidateId");
    }
    if (!inputExt.orgId) {
      throw new Error("Missing required parameter: orgId");
    }
    const pk = this.buildPK(inputExt.orgId as string, (inputExt.candidateId ?? inputExt.id ?? inputExt.candidateId) as string);
    // Soft-delete layout prefixes SK with ACTIVE# when statusPrefix is enabled.
    const skPrefix = `${this.ENTITY_TOKEN}#${(inputExt.candidateId ?? inputExt.id ?? inputExt.candidateId)}#`;
    ...
    if (!item) {
      throw new Error(`${this.ENTITY_TYPE} with id ${(inputExt.candidateId ?? inputExt.id ?? inputExt.candidateId)} not found`);
    }
    return {
