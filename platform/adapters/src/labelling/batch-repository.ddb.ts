/**
 * BatchRepository - DynamoDB Implementation
 *
 ...
 * BatchRepositoryDdb
 *
 * DDD: Infrastructure adapter implementing Batch repository using DynamoDB.
 * Generated from x-dynamodb metadata in OpenAPI schema.
 */

import type { BatchRepository } from "@cutpoint/services/labelling";
...
/**
 * BatchRepositoryDdb using DynamoDB table (type: core)
 *
 * Entity Type: ANNOTATION_BATCH
 * Entity Token: ANNOTATION_BATCH
 * SK Prefix: ANNOTATION_BATCH
 */
export class BatchRepositoryDdb implements BatchRepository {
  private readonly TABLE_NAME: string;
  // Metadata from x-dynamodb annotation - all values are dynamic
  private readonly ENTITY_TYPE = "ANNOTATION_BATCH";
  private readonly ENTITY_TOKEN = "ANNOTATION_BATCH";
  private readonly SK_PREFIX = "ANNOTATION_BATCH";
  ...
  private readonly DOMAIN = "LABELLING";
  private readonly ENTITY_TYPE_PK = "ANNOTATION_BATCHES";
  private generateCorrelationId(): string {
    const domainPrefix = "lbl";
  ...
  async createAnnotationBatch(input: Parameters<BatchRepository['createAnnotationBatch']>[0]): Promise<Awaited<ReturnType<BatchRepository['createAnnotationBatch']>>> {
    ...
    const batchId = inputExt?.batchId ?? inputExt?.id;
    if (!batchId) throw new Error("Missing required parameter: batchId (inject from use case)");
    const pk = this.buildPK(orgId as string, batchId as string);
    const sk = this.buildSK(batchId as string);
    const excludedFields = ["batchId" as string, "id"];
