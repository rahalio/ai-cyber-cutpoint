...
export function getDynamoClientInstance(): AdapterDynamoDBClient {
  if (!dynamoClientInstance) {
    if (isSandboxDynamoMode()) {
      dynamoClientInstance = new InMemoryDynamoClient();
...
const DOMAIN_PATH_PATTERNS: Array<{ pattern: RegExp; domain: string }> = [
  ...
  { pattern: /\/discovery\//, domain: "discovery" },
  { pattern: /\/governance\//, domain: "governance" },
  { pattern: /\/identity\//, domain: "identity" },
  { pattern: /\/interventions\//, domain: "interventions" },
  { pattern: /\/labelling\//, domain: "labelling" },
  { pattern: /\/markets\//, domain: "markets" },
];
...
export async function injectDomainDependencies(
  ...
  if (!domainDeps) {
    ...
    switch (domain) {
      case "discovery":
        domainDeps = buildDiscoveryDomainModule(dynamoClient);
        break;
      case "governance":
      ...
      case "identity":
        ...
        break;
      case "interventions":
        domainDeps = buildInterventionsDomainModule(dynamoClient);
        break;
      case "labelling":
        domainDeps = buildLabellingDomainModule(dynamoClient);
        break;
      case "markets":
        domainDeps = buildMarketsDomainModule(dynamoClient);
        break;
      default:
        return;
    }
...
export interface DomainDependencies {
  discovery: DiscoveryDomainModule;
  governance: GovernanceDomainModule;
  identity: IdentityDomainModule;
  interventions: InterventionsDomainModule;
  labelling: LabellingDomainModule;
  markets: MarketsDomainModule;
}
 ...
 */
export function getDiscoveryDeps(
  request: FastifyRequest,
): DomainDependencies["discovery"] {
  return getDomainDeps(request, "discovery");
}
...
export function getInterventionsDeps(
  request: FastifyRequest,
): DomainDependencies["interventions"] {
  return getDomainDeps(request, "interventions");
}
export function getLabellingDeps(
  request: FastifyRequest,
): DomainDependencies["labelling"] {
  return getDomainDeps(request, "labelling");
}
export function getMarketsDeps(
  request: FastifyRequest,
): DomainDependencies["markets"] {
  return getDomainDeps(request, "markets");
}
